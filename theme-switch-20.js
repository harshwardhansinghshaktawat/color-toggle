(function() {
    ...
    class ThemeSwitcherElement extends HTMLElement {
        ...
        attributeChangedCallback(name, oldValue, newValue) {
            if (newValue && newValue !== oldValue && name === 'settings') {
                try {
                    ...
                    // Re-render if flipIcons changed
                    if (this.isInitialized && oldValue) {
                        const oldSettingsObj = JSON.parse(oldValue);
                        if (oldSettingsObj.flipIcons !== newSettings.flipIcons) {
                            this.render();
                        }
                    }
                } catch (e) {
        ...
        render() {
            const sunIcon = '☀️';
            const moonIcon = '🌙';
            const leftIcon = this.settings.flipIcons ? moonIcon : sunIcon;
            const rightIcon = this.settings.flipIcons ? sunIcon : moonIcon;
            ...
            this.innerHTML = `
                ...
                <div class="theme-switcher-container">
                    ...
                    <span class="theme-icon">${leftIcon}</span>
                    ...
                    <span class="theme-icon">${rightIcon}</span>
                </div>
            `;
        ...
        setupToggleListener() {
            ...
            if (toggle) {
                // Set initial state based on current theme and flip setting
                const isDark = this.settings.currentTheme === 'dark';
                toggle.checked = this.settings.flipIcons ? !isDark : isDark;
                
                toggle.addEventListener('change', (e) => {
                    const isChecked = e.target.checked;
                    
                    // Determine theme based on flip setting
                    if (this.settings.flipIcons) {
                        this.settings.currentTheme = isChecked ? 'light' : 'dark';
                    } else {
        ...
        convertColor(colorString, toDark) {
            ...
            if (toDark) {
                if (brightness > 240) return targetColors[0];
                if (brightness > 220) return targetColors[1];
                if (brightness > 200) return targetColors[2];
                if (brightness > 180) return targetColors[3];
                if (brightness > 160) return targetColors[4];
                if (brightness > 140) return targetColors[5];
                if (brightness > 120) return targetColors[6];
                if (brightness > 100) return targetColors[7];
                if (brightness > 80) return targetColors[8];
                if (brightness < 60) return targetColors[10];
                return targetColors[9];
            } else {
                if (brightness < 20) return targetColors[0];
                if (brightness < 40) return targetColors[1];
                if (brightness < 60) return targetColors[2];
                if (brightness < 80) return targetColors[3];
                if (brightness < 100) return targetColors[4];
                if (brightness < 120) return targetColors[5];
                if (brightness < 140) return targetColors[6];
                if (brightness < 160) return targetColors[7];
                if (brightness < 180) return targetColors[8];
                if (brightness > 220) return targetColors[10];
                return targetColors[9];
            }
        }
