class Product360Viewer extends HTMLElement {
        ...
        this._shadow.innerHTML = `
            <style>
                * {
                ...
                /* FIXED: Hide the entire frame-info container */
                .frame-info.hidden {
                ...
                /* Control Buttons - Bottom Right Vertical */
                .control-buttons {
                    position: absolute;
                    bottom: 20px;
                    right: 20px;
    ...
    static get observedAttributes() {
        return [
            ...
            'viewer-title', 'title-font-family', 'title-tag', 'title-font-size'
        ];
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        if (name === 'frames-data' && newValue) {
            ...
            console.log('360 Viewer: Show frame number changed to:', this._settings.showFrameNumber);
        ...
        } else if (name === 'title-tag' && newValue) {
            this._settings.titleTag = newValue;
            this._updateTitle();
    ...
    _loadFrames(frameUrls) {
        if (!frameUrls || frameUrls.length === 0) {
            this._showError("No frames available for this product");
            return;
        }
        ...
        container.innerHTML = `
            ...
            <${this._settings.titleTag} class="viewer-title">${this._settings.viewerTitle}</${this._settings.titleTag}>
    ...
    _updateTitle() {
        const oldTitle = this._shadow.querySelector('.viewer-title');
        if (oldTitle) {
            const newTitle = document.createElement(this._settings.titleTag);
            newTitle.className = 'viewer-title';
            newTitle.textContent = this._settings.viewerTitle;
    ...
    _updateFrameNumberVisibility() {
        ...
        if (frameInfo) {
            if (this._settings.showFrameNumber) {
                ...
                console.log('360 Viewer: Frame info shown');
            } else {
                frameInfo.classList.add('hidden');
                console.log('360 Viewer: Frame info hidden');
            }
        } else {
            console.warn('360 Viewer: frame-info element not found');
        }
    }
